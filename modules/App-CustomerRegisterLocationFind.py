...
# -----------------------------------
# Pincode -> State ranges
# -----------------------------------
# NOTE: Stored as a LIST of (start, end, state) tuples rather than a dict.
# A dict keyed by (start, end) silently drops earlier entries whenever the
# exact same range is reused for two different states (this happened for
# Uttar Pradesh / Uttarakhand and Tamil Nadu / Puducherry in the original
# version), so real matches were being mislabeled without any error.
# Overlaps are resolved by get_state_from_pincode() picking the NARROWEST
# matching range (i.e. the most specific one wins), so a small enclave
# range (like a Puducherry pocket) correctly overrides a broader
# surrounding state range instead of being silently discarded.
...
# ---- Run detection with a real progress bar ----
