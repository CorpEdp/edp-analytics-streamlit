...
#
# FIX: previously there were two independently-maintained dicts
# (BRANCH_DISPLAY_NAMES here, and a separate `full_name_map`
# inside extract_branch_code()) describing the same code<->name
# relationship. They had already drifted - e.g. the
# "VIRUDHURNGAR" typo only existed in one of them. Now there is
# ONE dict (BRANCH_CODE_TO_NAME) and everything else is derived
# from it, so the two directions can never diverge again.
# ============================================================

# code -> canonical display name (the ONLY place codes are defined)
BRANCH_CODE_TO_NAME = {
...
# derived reverse map: normalized full-name text -> code
_FULL_NAME_TO_CODE = {
    name.upper(): code
...
# BRANCH_DISPLAY_NAMES directly (e.g. get_branch_display_name).
# Derived from BRANCH_CODE_TO_NAME, not hand-maintained separately.
...
# ITEM DETECTION
#
# FIX: the original used bare substring checks like
# `"V-" in label_str`, `"R-" in label_str`, `"P-" in label_str`.
# These match ANYWHERE in the string, not just as a leading
# branch/category code, so a label like "COVER-100" or
# "SUPER-DEAL" would have silently been misclassified (matching
# "V-"/"P-" mid-word). Now these short codes must appear as a
# standalone token - at the start of the label, or right after a
# space/slash/comma - never mid-word.
#
# Also removed the old dead/unsafe fallback block that re-checked
# loose single-letter-plus-space patterns (" G", " S", " P", " D"):
# it was unreachable in almost all cases because the primary
# rules above it already cover GOLD/SILVER/PLATINUM/DIAMOND
# keywords, and dangerously broad on the rare case it was reached.
# ============================================================
...
# ============================================================
# BRANCH CODE EXTRACTION - IMPROVED
#
# FIX: now reads from the single _FULL_NAME_TO_CODE map defined
# above instead of maintaining its own separate `full_name_map`.
# ============================================================
