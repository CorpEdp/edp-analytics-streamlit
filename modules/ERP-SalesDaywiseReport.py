...
# ============================================================
# BRANCH CODE <-> DISPLAY NAME (single source of truth)
# ============================================================
...
def auto_detect_columns(df):
    ...
    def find(include_any=None, include_all=None, exclude=None, exact=None):
    ...
    date_col = find(exact="date", include_any=["date"])

    branch_col = find(exact="branch location", include_all=["branch"], exclude=["label"])
    if branch_col is None:
        branch_col = find(include_any=["branch"], exclude=["label"])

    label_col = find(exact="label location", include_any=["label", "counter"])

    amount_col = find(exact="item amount", include_all=["item", "amount"])
    if amount_col is None:
        amount_col = find(include_all=["taxable", "amount"])
    if amount_col is None:
        amount_col = find(include_any=["amount", "taxable"])

    gross_wt_col = find(exact="gross wt", include_all=["gross"], exclude=["diamond"])
    if gross_wt_col is None:
        gross_wt_col = find(exact="gross weight", include_all=["gross"], exclude=["diamond"])
    if gross_wt_col is None:
        gross_wt_col = find(include_any=["gross"], exclude=["diamond"])

    diamond_wt_col = find(exact="diamond weight ct", include_all=["diamond", "weight"])
    if diamond_wt_col is None:
        diamond_wt_col = find(exact="diamond weight (ct)", include_all=["diamond", "weight"])
    if diamond_wt_col is None:
        diamond_wt_col = find(include_all=["diamond", "weight"])
    if diamond_wt_col is None:
        diamond_wt_col = find(include_all=["diamond", "wt"])
    if diamond_wt_col is None:
        diamond_wt_col = find(include_all=["stone", "weight"])
    if diamond_wt_col is None:
        diamond_wt_col = find(include_any=["diamond"])

    diamond_amount_col = find(exact="diamond amount", include_all=["diamond", "amount"])
    if diamond_amount_col is None:
        diamond_amount_col = find(exact="stone amount", include_all=["stone", "amount"])
    if diamond_amount_col is None:
        diamond_amount_col = find(include_all=["diamond", "amount"])
    if diamond_amount_col is None:
        diamond_amount_col = find(include_all=["diamond", "amt"])
    if diamond_amount_col is None:
        diamond_amount_col = find(include_all=["stone", "amount"])
    if diamond_amount_col is None:
        diamond_amount_col = find(include_any=["stone", "amount", "diamond", "amt"])
...
def build_single_sheet_report(
    ...
    # ========================================================
    # DATE RANGE (for title & filename)
    # ========================================================

    valid_dates = sorted(
        d for d in df_filtered["Date"].unique()
    ...
    branch_font = Font(bold=True, size=11, color="FFFFFF")
    branch_fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid")
    ...
    grand_total_fill = PatternFill(start_color="C00000", end_color="C00000", fill_type="solid")
    ...
    def branch_item_totals(data):
        ...
        for b in branches:
            ...
            bdf = data[data["Branch_Code"] == b]

            for item in items:

                idf = bdf[bdf["Item"] == item]
                ...
                if item == "Gold":
                    ...
                    gold_df = bdf[bdf["Item"] == "Gold"]
                    gold_weight = (
                        float(gold_df[gross_wt_col].sum())
                        if len(gold_df)
                        else 0.0
                    )
                    ...
                    diamond_weight = (
                        float(bdf[bdf[diamond_wt_col] > 0][diamond_wt_col].sum())
                        if len(bdf)
                        else 0.0
                    )

                    if diamond_amount_col and diamond_amount_col in idf.columns:
                        diamond_amount = (
                            float(idf[diamond_amount_col].sum())
                            if len(idf)
                            else 0.0
                        )
                    else:
        ...
        for b in branches:
            for item in items:
                wt = branch_weights.get((b, item), 0.0)
                item_totals[item] = item_totals.get(item, 0.0) + wt

        for b in branches:
            for item in items:
                wt = branch_weights.get((b, item), 0.0)
                denominator = item_totals.get(item, 0.0)
                pct = (
                    wt / denominator * 100
                    if denominator > 0
                    else 0.0
                )
                result[(b, item)]["pct"] = pct
    ...
    def write_sub_header_row(
        ...
        c0 = ws.cell(row=row, column=1, value=first_col_label)
        c0.font = header_font
        c0.fill = header_fill
        c0.alignment = center
        c0.border = border
        ...
        if first_branch_col == 3:
            c1 = ws.cell(row=row, column=2, value=second_col_label)
            c1.font = header_font
            c1.fill = header_fill
            c1.alignment = center
            c1.border = border
        ...
        for _ in branches:
            ...
            cell = ws.cell(row=row, column=col, value="Branch")
            ...
            for header in headers[1:]:
                ...
                if "Wt" in header or "Weight" in header:
                    cell.fill = PatternFill(
                        start_color="E2EFDA",
                        end_color="E2EFDA",
                        fill_type="solid"
                    )
                elif "Amt" in header or "Amount" in header:
                    cell.fill = PatternFill(
                        start_color="FFEB9C",
                        end_color="FFEB9C",
                        fill_type="solid"
    ...
    def write_totals_row(
        ...
        label_cell = ws.cell(row=row, column=1, value=row_label)
        label_cell.alignment = center
        label_cell.border = thick_border if is_grand else border
        ...
        if first_branch_col == 3:
            day_cell = ws.cell(row=row, column=2, value=day_name)
        ...
        for b in branches:
            ...
            branch_cell.font = Font(
                bold=True,
                size=9,
                color="000000" if not is_grand else "FFFFFF"
            )
            col += 1

            # GOLD
            gold_data = totals.get((b, "Gold"), {})
            gold_wt = gold_data.get("wt", 0.0)
            gold_amt = gold_data.get("amt", 0.0)
            gold_pct = gold_data.get("pct", 0.0)

            values = [gold_wt, gold_amt, 100.0 if is_grand else gold_pct]

            for idx, value in enumerate(values):
                cell = ws.cell(row=row, column=col, value=round(value, 2))
                cell.alignment = center if idx == 2 else right
                cell.border = thick_border if is_grand else border
            ...
            # SILVER
            silver_data = totals.get((b, "Silver"), {})
            silver_wt = silver_data.get("wt", 0.0)
            silver_amt = silver_data.get("amt", 0.0)
            silver_pct = silver_data.get("pct", 0.0)

            values = [silver_wt, silver_amt, 100.0 if is_grand else silver_pct]
            ...
            # DIAMOND
            diamond_data = totals.get((b, "Diamond"), {})
            diamond_gross_wt = diamond_data.get("gross_wt", 0.0)
            diamond_wt = diamond_data.get("stone_wt", 0.0)
            diamond_amt = diamond_data.get("diamond_amt", 0.0)
            diamond_item_amt = diamond_data.get("item_amt", 0.0)
            diamond_pct = diamond_data.get("pct", 0.0)
            ...
            for idx, value in enumerate(values):
                cell = ws.cell(row=row, column=col, value=round(value, 2))
                cell.alignment = center if idx == 4 else right
                cell.border = thick_border if is_grand else border
            ...
            # PLATINUM
            platinum_data = totals.get((b, "Platinum"), {})
            platinum_wt = platinum_data.get("wt", 0.0)
            platinum_amt = platinum_data.get("amt", 0.0)
            platinum_pct = platinum_data.get("pct", 0.0)

            values = [platinum_wt, platinum_amt, 100.0 if is_grand else platinum_pct]
    ...
    title_text = "📊 BRANCH-WISE SALES REPORT – CUMULATIVE & DAILY"
    if date_range_str:
    ...
    cumulative_totals = branch_item_totals(df_filtered)
    ...
    c = ws.cell(row=row, column=1, value="📅 DAY WISE SALES REPORT")
    c.font = section_font
    c.fill = section_fill
    c.alignment = center
    ...
    for d in dates:
        ...
        day_df = df_filtered[df_filtered["Date"] == d]
        ...
        day_totals = branch_item_totals(day_df)
    ...
    ws.freeze_panes = ws.cell(row=date_header_row + 1, column=3)
    ...
    gold_only_df = df_filtered[df_filtered["Item"] == "Gold"].copy()

    gold_branches = sort_branches(gold_only_df["Branch_Code"].unique())

    gold_cols_per_branch = 4
    gold_total_cols = 2 + len(gold_branches) * gold_cols_per_branch
    ...
    def gold_totals(data):
        ...
        for branch in gold_branches:
            ...
            totals[branch] = {
                "wt": weight,
                "amt": amount,
                "pct": weight / total_weight * 100 if total_weight > 0 else 0.0,
            }
    ...
    def write_gold_row(row, totals, label, day_name="", grand=False):
        ...
        for branch in gold_branches:
            ...
            cell = ws2.cell(
                row=row,
                column=col,
                value=get_branch_display_name(branch)
            )
            cell.alignment = center
            cell.border = thick_border if grand else border
            cell.fill = PatternFill(
            ...
            cell.font = Font(
                bold=True,
                size=9,
                color="000000" if not grand else "FFFFFF"
            )
            col += 1
    ...
    gold_title = "🥇 GOLD SALES REPORT – CUMULATIVE & DAILY"
    if date_range_str:
        gold_title += f"   {date_range_str}"

    title = ws2.cell(row=row2, column=1, value=gold_title)
    ...
    for date_value in sorted(gold_only_df["Date"].unique()):
        ...
        day_data = gold_only_df[gold_only_df["Date"] == date_value]
        write_gold_row(
            row2,
    ...
    ws2.freeze_panes = ws2.cell(row=gold_date_header_row + 1, column=3)
    ...
    # ========================================================
    # SAVE - return dict with bytes + date range info
    # ========================================================
...
def process_data(
    ...
    if fill_blanks:
        df_clean, filled_count = fill_blank_branches(df_clean, branch_col)
    ...
    df_clean[gross_wt_col] = clean_numeric_series(df_clean[gross_wt_col])
    df_clean[diamond_wt_col] = clean_numeric_series(df_clean[diamond_wt_col])
    df_clean[amount_col] = clean_numeric_series(df_clean[amount_col])

    # ========================================================
    # DIAMOND AMOUNT
    # ========================================================

    if diamond_amount_col and diamond_amount_col in df_clean.columns:
        df_clean[diamond_amount_col] = clean_numeric_series(df_clean[diamond_amount_col])
    else:
    ...
    df_filtered = df_clean[
        ~df_clean["Item"].isin(["Skip", "Unknown"])
    ].copy()
    ...
    for item in ["Gold", "Silver", "Diamond", "Platinum"]:
        item_data = df_filtered[df_filtered["Item"] == item]
        total_amounts[item] = (
            float(item_data[amount_col].sum())
            if len(item_data)
    ...
    unique_branches = sort_branches(df_filtered["Branch_Code"].unique())
    ...
    report_df = pd.DataFrame(index=unique_branches)

    # GOLD
    gold_data = df_filtered[df_filtered["Item"] == "Gold"]

    gold_weight = (
        gold_data.groupby("Branch_Code")[gross_wt_col].sum()
        if len(gold_data)
        else pd.Series(dtype=float)
    )

    gold_weight_series = pd.Series(0.0, index=unique_branches)
    gold_weight_series.update(gold_weight)
    report_df["Gold Wt (g)"] = gold_weight_series

    gold_amount = (
        gold_data.groupby("Branch_Code")[amount_col].sum()
        if len(gold_data)
        else pd.Series(dtype=float)
    )

    gold_amount_series = pd.Series(0.0, index=unique_branches)
    gold_amount_series.update(gold_amount)
    report_df["Gold Item Amt"] = gold_amount_series

    total_gold_weight = report_df["Gold Wt (g)"].sum()
    if total_gold_weight > 0:
        report_df["Gold %"] = (
            report_df["Gold Wt (g)"] / total_gold_weight * 100
        ).round(1)
    else:
        report_df["Gold %"] = 0.0

    # SILVER
    silver_data = df_filtered[df_filtered["Item"] == "Silver"]

    if len(silver_data) > 0:
        silver_weight = silver_data.groupby("Branch_Code")[gross_wt_col].sum()
        silver_amount = silver_data.groupby("Branch_Code")[amount_col].sum()
    else:
        silver_weight = pd.Series(dtype=float)
        silver_amount = pd.Series(dtype=float)

    silver_weight_series = pd.Series(0.0, index=unique_branches)
    silver_amount_series = pd.Series(0.0, index=unique_branches)
    silver_weight_series.update(silver_weight)
    silver_amount_series.update(silver_amount)

    report_df["Silver Wt (g)"] = silver_weight_series
    report_df["Silver Item Amt"] = silver_amount_series

    total_silver_weight = report_df["Silver Wt (g)"].sum()
    if total_silver_weight > 0:
        report_df["Silver %"] = (
            report_df["Silver Wt (g)"] / total_silver_weight * 100
        ).round(1)
    else:
        report_df["Silver %"] = 0.0

    # DIAMOND
    diamond_data = df_filtered[df_filtered["Item"] == "Diamond"]

    if len(diamond_data) > 0:
        diamond_gross_wt = diamond_data.groupby("Branch_Code")[gross_wt_col].sum()
        diamond_item_amount = diamond_data.groupby("Branch_Code")[amount_col].sum()

        if diamond_amount_col and diamond_amount_col in diamond_data.columns:
            diamond_amount = diamond_data.groupby("Branch_Code")[diamond_amount_col].sum()
        else:
            diamond_amount = pd.Series(0.0, index=unique_branches)
    else:
        diamond_gross_wt = pd.Series(dtype=float)
        diamond_item_amount = pd.Series(dtype=float)
        diamond_amount = pd.Series(dtype=float)

    diamond_gross_wt_series = pd.Series(0.0, index=unique_branches)
    diamond_item_amount_series = pd.Series(0.0, index=unique_branches)
    diamond_amount_series = pd.Series(0.0, index=unique_branches)
    diamond_gross_wt_series.update(diamond_gross_wt)
    diamond_item_amount_series.update(diamond_item_amount)
    diamond_amount_series.update(diamond_amount)

    report_df["Diamond Gross Wt (g)"] = diamond_gross_wt_series

    diamond_weight_all = (
        df_filtered[df_filtered[diamond_wt_col] > 0]
        .groupby("Branch_Code")[diamond_wt_col].sum()
    )

    diamond_weight_series = pd.Series(0.0, index=unique_branches)
    diamond_weight_series.update(diamond_weight_all)

    report_df["Diamond Wt (Ct)"] = diamond_weight_series
    report_df["Diamond Amt"] = diamond_amount_series
    report_df["Diamond Item Amt"] = diamond_item_amount_series

    total_diamond_weight = report_df["Diamond Wt (Ct)"].sum()
    if total_diamond_weight > 0:
        report_df["Diamond %"] = (
            report_df["Diamond Wt (Ct)"] / total_diamond_weight * 100
        ).round(1)
    else:
        report_df["Diamond %"] = 0.0

    # PLATINUM
    platinum_data = df_filtered[df_filtered["Item"] == "Platinum"]

    if len(platinum_data) > 0:
        platinum_weight = platinum_data.groupby("Branch_Code")[gross_wt_col].sum()
        platinum_amount = platinum_data.groupby("Branch_Code")[amount_col].sum()
    else:
        platinum_weight = pd.Series(dtype=float)
        platinum_amount = pd.Series(dtype=float)

    platinum_weight_series = pd.Series(0.0, index=unique_branches)
    platinum_amount_series = pd.Series(0.0, index=unique_branches)
    platinum_weight_series.update(platinum_weight)
    platinum_amount_series.update(platinum_amount)

    report_df["Platinum Wt (g)"] = platinum_weight_series
    report_df["Platinum Item Amt"] = platinum_amount_series

    total_platinum_weight = report_df["Platinum Wt (g)"].sum()
    if total_platinum_weight > 0:
        report_df["Platinum %"] = (
            report_df["Platinum Wt (g)"] / total_platinum_weight * 100
        ).round(1)
    else:
    ...
    report_display = report_display.reset_index()

    # ========================================================
    # GRAND TOTAL
    # ========================================================

    total_row = {"Branch": "GRAND TOTAL"}
    ...
    for col in report_display.columns:
        ...
        if "%" in col:
            ...
            total_row[col] = report_display[col].sum()

    report_display = pd.concat(
        [report_display, pd.DataFrame([total_row])],
        ignore_index=True
    )
    ...
    detection_stats = df_clean["Item"].value_counts()
    stats_data = []

    for cat in ["Gold", "Silver", "Diamond", "Platinum", "Skip", "Unknown"]:
        count = detection_stats.get(cat, 0)
    ...
    branch_mapping = (
        df_clean.groupby(branch_col)["Branch_Code"]
        .first()
        .reset_index()
    )
    ...
    unknown_data = df_clean[df_clean["Item"] == "Unknown"][
        [branch_col, label_col, "Item"]
    ].head(20)
    ...
    day_wise_weight = pd.DataFrame(
        index=sorted(df_filtered["Date"].unique())
    )

    for d in day_wise_weight.index:

        day_data = df_filtered[df_filtered["Date"] == d]

        gold_weight_day = day_data[day_data["Item"] == "Gold"][gross_wt_col].sum()
        diamond_gross_day = day_data[day_data["Item"] == "Diamond"][gross_wt_col].sum()
        diamond_weight_day = day_data[day_data[diamond_wt_col] > 0][diamond_wt_col].sum()
        silver_weight_day = day_data[day_data["Item"] == "Silver"][gross_wt_col].sum()
        platinum_weight_day = day_data[day_data["Item"] == "Platinum"][gross_wt_col].sum()

        day_wise_weight.loc[d, "Gold Wt (g)"] = gold_weight_day
        day_wise_weight.loc[d, "Silver Wt (g)"] = silver_weight_day
        day_wise_weight.loc[d, "Diamond Gross Wt (g)"] = diamond_gross_day
        day_wise_weight.loc[d, "Diamond Wt (Ct)"] = diamond_weight_day
        day_wise_weight.loc[d, "Platinum Wt (g)"] = platinum_weight_day
    ...
    for item in ["Gold", "Silver", "Diamond", "Platinum"]:
        ...
        if item == "Gold":
            ...
            diamond_weight_ct = float(
                df_filtered[df_filtered[diamond_wt_col] > 0][diamond_wt_col].sum()
            )
    ...
    # ========================================================
    # EXCEL REPORT - capture returned dict
    # ========================================================

    report_output = build_single_sheet_report(
        df_filtered,
        amount_col,
        gross_wt_col,
        diamond_wt_col,
    ...
    formatted_bytes = report_output["bytes"]
    report_date_range_str = report_output["date_range_str"]
    report_start_str = report_output["start_str"]
    report_end_str = report_output["end_str"]
    ...
    sample_data = df_clean[
        [branch_col, label_col, "Branch_Code", "Item"]
    ].head(10)
    ...
    return {
        ...
        "n_branches": df_filtered["Branch_Code"].nunique(),
        ...
        "report_date_range_str": report_date_range_str,

        "report_start_str": report_start_str,

        "report_end_str": report_end_str,
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 💎 Diamond Weight Rule")
    st.info(
        """
    ...
    fill_blanks = st.checkbox(
        ...
        value=True,
        help="Fill blank branch cells using the previous branch name"
    )
...
if uploaded_file is not None:
    ...
    try:
        ...
        if (
            st.session_state.get("_last_uploaded_name")
            != uploaded_file.name
        ):
        ...
        with col3:
            if "Branch Location" in df.columns:
                blank_count = (
                    df["Branch Location"].isna().sum()
                    + (
                        df["Branch Location"].astype(str).str.strip() == ""
                    ).sum()
                )
        ...
        guess = auto_detect_columns(df)
        cols_list = list(df.columns)

        def idx(colname):
            if colname is not None and colname in cols_list:
                return cols_list.index(colname)
            return 0
        ...
        st.markdown("#### 📦 Product Item Column")
        ...
        item_col_options = ["None"] + cols_list
        ...
        st.markdown("#### 💎 Diamond Amount Column")

        diamond_amount_options = ["None"] + cols_list

        detected_diamond_amount = (
            guess["diamond_amount"]
            if guess["diamond_amount"] in cols_list
            else "None"
        )
        ...
        diamond_amount_col_selected = st.selectbox(
            ...
            index=(
                diamond_amount_options.index(detected_diamond_amount)
                if detected_diamond_amount in diamond_amount_options
                else 0
            ),
            help="Select the Diamond Amount column. For example: Diamond Amount = ₹5,740.00"
        )

        if diamond_amount_col_selected == "None":
            diamond_amount_col = None
        else:
            diamond_amount_col = diamond_amount_col_selected
        ...
        if st.button(
            ...
            with st.spinner("Analyzing and generating reports..."):
                ...
                try:
                    ...
                    st.session_state["report_generated"] = False
                    st.error(f"❌ Error while generating report: {gen_err}")
        ...
        if (
            ...
            if results["filled_count"] > 0:
                st.success(
                    f"✅ Filled {results['filled_count']} "
            ...
            if len(results["unknown_data"]) > 0:
                with st.expander(
                    f"⚠️ {len(results['unknown_data'])} "
                    "Unknown Items - Click to view"
                ):
                    st.dataframe(results["unknown_data"])

            st.success(
                f"✅ Processing {results['n_rows_filtered']} rows "
            ...
            st.subheader("🏢 Branch-wise Cumulative Report")

            report_display = results["report_display"]
            ...
            for col in report_display.columns:
                ...
                if "%" in col:
                    ...
                    format_dict[col] = "₹{:,.2f}"
                elif "Wt" in col or "Weight" in col:
                    format_dict[col] = "{:,.3f}"
            ...
            report_display.to_csv(csv_buffer, index=False)
            ...
            st.subheader("📊 Branch-wise Visualization")

            chart_data = report_display[
                report_display["Branch"] != "GRAND TOTAL"
            ].copy()

            if len(chart_data) > 0:

                pct_cols = ["Gold %", "Silver %", "Diamond %", "Platinum %"]
                pct_cols_exist = [c for c in pct_cols if c in chart_data.columns]

                if (
                    pct_cols_exist
                    and chart_data[pct_cols_exist].sum().sum() > 0
                ):
                    st.markdown("#### Percentage Distribution by Branch")
                    st.bar_chart(
                        chart_data.set_index("Branch")[pct_cols_exist]
                    )
                ...
                weight_cols_exist = [c for c in weight_cols if c in chart_data.columns]

                if (
                    weight_cols_exist
                    and chart_data[weight_cols_exist].sum().sum() > 0
                ):
                    st.markdown("#### Weight by Branch")
                    st.bar_chart(
                        chart_data.set_index("Branch")[weight_cols_exist]
                    )
                ...
                amount_cols_exist = [c for c in amount_cols if c in chart_data.columns]

                if (
                    amount_cols_exist
                    and chart_data[amount_cols_exist].sum().sum() > 0
                ):
                    st.markdown("#### Item Amount by Branch")
                    st.bar_chart(
                        chart_data.set_index("Branch")[amount_cols_exist]
                    )
            ...
            if len(results["day_wise"]) > 0:

                day_tab1, day_tab2 = st.tabs(["💰 Amount", "⚖️ Weight"])
            ...
            # =================================================
            # DOWNLOAD EXCEL  (with date range in filename)
            # =================================================

            st.divider()
            st.subheader("📥 Download Formatted Report")

            start_str = results.get("report_start_str", "")
            end_str = results.get("report_end_str", "")

            if start_str and end_str:
                excel_filename = (
                    f"Branch-wise Sales Report – Cumulative & Daily  "
                    f"{start_str} to {end_str}.xlsx"
                )
            else:
                excel_filename = (
                    "Branch-wise Sales Report – Cumulative & Daily.xlsx"
                )

            # Sanitize illegal filename characters (Windows / macOS)
            excel_filename = re.sub(r'[\\/:*?"<>|]', "-", excel_filename)

            st.download_button(
                label="📥 Download Branch-wise Excel Report",
                data=results["formatted_bytes"],
            ...
            total_by_item = results["total_by_item"]
            ...
            for metric_idx, item in enumerate(
                ...
                with metrics_cols[metric_idx]:

                    if item in total_by_item.index:

                        item_amount = total_by_item.loc[item, "Item Amount"]
                        weight = total_by_item.loc[item, "Weight"]
                        diamond_amount = total_by_item.loc[item, "Diamond Amount"]
...
# ============================================================
    ...
    with st.expander("📋 How This Works"):
        ...
        st.markdown(
            ...
            **Gold** - Gold Wt (g), Gold Item Amt, Gold %
            **Silver** - Silver Wt (g), Silver Item Amt, Silver %
            **Diamond** - Diamond Gross Wt (g), Diamond Wt (Ct), Diamond Amt, Diamond Item Amt, Diamond %
            **Platinum** - Platinum Wt (g), Platinum Item Amt, Platinum %
